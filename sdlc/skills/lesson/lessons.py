...
Exit codes:
    ...
    4 — the queue was written by a NEWER plugin than this copy (its
        lessons_file_version is ahead): nothing was checked or written, and
        the one line printed says to run /sdlc:setup. The file is not at
        fault - an older emitter would drop fields it does not know, and an
        older validator would blame the file for a status the newer plugin
        defined (0.9.16's validate printed 109 MUST FIX rows on a queue
        0.9.17 had stamped).
"""
...
# The queue's own format version, stamped by every write. Bump it whenever an
# enum this file validates widens or a field the emitter must carry is added:
# LESSON_STATUSES widened at 1.17 without a bump, so a 0.9.16 helper read a
# 0.9.17-stamped queue as 109 schema violations and blamed the file. A helper
# that finds a queue AHEAD of this number says so in one line (exit 4) and
# writes nothing - `file_version_ahead` is that check, at every write site and
# in validate.
#   "1"  statuses open | collected
#   "2"  the six statuses + imp_id / fixed_in / verdict_at (the verdict loop)
...
def _empty_queue() -> dict:
    return {
        "lessons_file_version": LESSONS_FILE_VERSION,
        "last_updated": None,
        "last_ids": {"LSN": 0},
        "runs": [],
        "lessons": [],
...
def _file_version_tuple(value) -> "tuple[int, ...] | None":
    try:
...
def file_version_ahead(doc: dict) -> "tuple[str, str] | None":
    """(the queue's lessons_file_version, this helper's) when the queue was
    written by a NEWER plugin than this copy - None otherwise, a version that
    does not parse included (the validator reports that as its own error).
    Every write site and `validate` ask this first: an older emitter would
    drop the fields it does not know, and an older checker would blame the
    file for an enum value the newer plugin defined."""
    theirs = _file_version_tuple(doc.get("lessons_file_version"))
    mine = _file_version_tuple(LESSONS_FILE_VERSION)
    if theirs is None or mine is None or theirs <= mine:
        return None
    return str(doc.get("lessons_file_version")).strip(), LESSONS_FILE_VERSION


def newer_queue_line(path, ahead: "tuple[str, str]") -> str:
    """The one line an older helper prints about a newer queue, and nothing
    else: never a MUST FIX list, never 'the collector would refuse it' (the
    collector is the newer side)."""
    theirs, mine = ahead
    return (f"{path} was written by a newer plugin than this copy of lessons.py "
            f"(lessons_file_version {theirs} > {mine}) - the file is not at fault; "
            f"run /sdlc:setup once to update .claude/sdlc/lessons.py")
...
def dump_queue(doc: dict, path: Path) -> None:
    ...
    # This emitter defines the file's shape, so the file carries THIS version:
    # an older queue is upgraded on its first write, a newer one never reaches
    # here (file_version_ahead refuses the write first).
    lines.append("lessons_file_version:")
    _emit(LESSONS_FILE_VERSION, 0, lines)
...
def reconcile(root: Path, plugin_root, today: "str | None" = None) -> dict:
    ...
    Returns {status: no-manifest | unreadable | newer | no-rows | ok, path,
    queue, stamped: {status: n}, stamped_ids: {status: [lsn_id]}, reopened: n,
    reopened_ids: [{lsn_id, imp_id, fixed_in, last_seen_at, occurrences}],
    written: bool, problem}. `newer` means the queue was written by a newer
    plugin than this helper (file_version_ahead) and was left alone. Never
    raises for a missing or unreadable manifest - a close must not fail on it.
    """
    out = {"status": "no-manifest", "path": None, "queue": Path(root) / QUEUE_REL,
           "stamped": {}, "stamped_ids": {}, "reopened": 0, "reopened_ids": [],
           "written": False, "problem": None}
    if not plugin_root:
        return out
    ...
    try:
        ...
        out.update(status="unreadable", problem=str(e))
        return out
    ahead = file_version_ahead(queue)
    if ahead:
        out.update(status="newer", problem=newer_queue_line(out["queue"], ahead))
        return out
    ...
    stamped_ids: "dict[str, list[str]]" = {}
    for lesson in named:
        result = apply_verdict(lesson, mine[str(lesson["lsn_id"])], today)
        if result == "reopened":
            out["reopened"] += 1
            # The ids, with the verdict they recurred after: a count alone
            # sent the reader into the queue to find which lesson to reopen.
            out["reopened_ids"].append({
                "lsn_id": str(lesson.get("lsn_id")),
                "imp_id": lesson.get("imp_id"),
                "fixed_in": lesson.get("fixed_in"),
                "last_seen_at": lesson.get("last_seen_at") or lesson.get("raised_at"),
                "occurrences": _occurrences(lesson),
            })
        elif result:
            stamped[result] = stamped.get(result, 0) + 1
            stamped_ids.setdefault(result, []).append(str(lesson.get("lsn_id")))
    out["stamped"] = stamped
    out["stamped_ids"] = stamped_ids
    if stamped:
...
def verdict_summary(summary: dict) -> str:
    ...
    # Ids, not only counts: this line is the one place a close names which
    # lessons the maintainer closed and which one is a REOPEN candidate, and a
    # bare "1 left open" sent the reader into the queue to find it.
    ids = summary.get("stamped_ids") or {}
    parts = "; ".join(f"{', '.join(ids.get(k) or [])} {k}" if ids.get(k) else f"{stamped[k]} {k}"
                      for k in VERDICT_STATUSES if stamped.get(k))
    ...
    if summary["reopened"]:
        ...
        named = []
        for r in summary.get("reopened_ids") or []:
            facts = [str(r["imp_id"])] if r.get("imp_id") else []
            if r.get("fixed_in"):
                facts.append(f"fixed_in {r['fixed_in']}")
            if r.get("last_seen_at"):
                facts.append(f"last seen {str(r['last_seen_at'])[:10]}")
            facts.append(f"{r.get('occurrences') or 1} report(s)")
            named.append(f"{r['lsn_id']} ({', '.join(facts)})")
        if named:
            text += ": " + "; ".join(named)
...
def flush(root: Path, force: bool = False, timeout: float = 5.0, quiet: bool = False) -> bool:
    ...
    try:
        ...
        return False
    if file_version_ahead(queue):
        # A newer plugin's queue: this copy cannot stamp sent_at without
        # dropping what it does not know. The next /sdlc:setup fixes it; a
        # flush is never the place to say so.
        return False
...
def cmd_record_run(args) -> int:
    ...
    ahead = file_version_ahead(queue)
    if ahead:
        print(f"[FAIL] no run recorded - {newer_queue_line(queue_path, ahead)}")
        return 4
...
def cmd_add(args) -> int:
    ...
    ahead = file_version_ahead(queue)
    if ahead:
        print(f"[FAIL] lesson not recorded - {newer_queue_line(queue_path, ahead)}")
        return 4
...
def cmd_list(args) -> int:
    ...
    if getattr(args, "json", False):
        # The lesson dicts as stored - status, imp_id, fixed_in, verdict_at
        # included - so a project's own join over the verdicts reads this
        # instead of parsing the queue YAML itself.
        print(json.dumps(lessons, indent=2, ensure_ascii=False, default=str))
        return 0
...
def cmd_validate(args) -> int:
    ...
    ahead = file_version_ahead(queue)
    if ahead:
        # One line, a distinct exit, and no MUST FIX list: the enum values
        # this copy does not know are the newer plugin's, not the file's.
        print(f"[FAIL] not checked - {newer_queue_line(path, ahead)}")
        print_next("run validate again after /sdlc:setup has updated the helper.")
        return 4
...
def cmd_reconcile(args) -> int:
    ...
    if summary["status"] == "unreadable":
        ...
        return 2
    if summary["status"] == "newer":
        print(f"[FAIL] nothing stamped - {summary['problem']}")
        return 4
...
def main(argv=None) -> int:
    ...
    p.add_argument("--json", action="store_true",
                   help="Print the matching lessons as a JSON array (every stored field, "
                        "the verdict fields included) instead of the one-line-per-lesson form.")
