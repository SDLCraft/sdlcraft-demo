...
- `lessons.py` `stamp_recurrence` (ledger IMP-231, aicf LSN-100 dated at its first sighting across a fix): each recurrence appends `sightings: [{at, plugin_version, installed_version, skill_version}]` (newest last, capped at 5); the top-level versions stay the first sighting's. The maintainer's collector reads the newest sighting for the delta flag and the REOPEN line, says "unknown" when a repeat carries none, and its relay fold unions `sightings` and keeps `last_seen_at`. `LESSONS.schema.yaml` documents both optional fields; no queue version bump. Pinned by `_smoke/lessons_selftest.py` and `_smoke/collect_selftest.py`.

## 1.18 (2026-09-24) — A queue a newer plugin wrote is refused in one line (exit 4), never blamed: `lessons_file_version` moves to "2" and every write site compares it; the `Verdicts:` line names the stamped and reopened ids; `list --json`

- `lessons.py`: `LESSONS_FILE_VERSION = "2"`, stamped by every write (an older queue is upgraded on its first write). `file_version_ahead` runs first in `validate`, `record-run`, `add`, `reconcile` and the delivery flush: a queue whose version is ahead of the helper's gets ONE line naming the two versions and `/sdlc:setup`, exit 4, nothing checked, nothing written - never a MUST FIX list, never "the maintainer's collector would refuse it" (the 0.9.16 copy installed in a project printed 109 of those on the queue 0.9.17 had stamped; ledger IMP-214). At the same version an unknown status is still the schema violation it was. Fixture `_smoke/09_newer_file_version.yaml` (expect 4); `lessons_selftest.py` file-version arm; `collect_lessons.py queue_errors` accepts a newer report unchecked and prints `newer_report_line` (`collect_selftest.py check_newer_report`).
- `SKILL.md` (ledger IMP-218): the commit step's helper line says every `python .claude/sdlc/<helper>.py` in the file runs the copy `helper-resolution.md` picks once per run (this skill's own `lessons.py` is always the plugin's).
- `reconcile` / `record-run`'s `Verdicts:` line names every stamped id beside its verdict and every reopened id with its `imp_id`, `fixed_in`, last-seen date and report count - a bare "1 left open" sent the reader into the queue to find which lesson to reopen (ledger IMP-219). `list --json` prints the matching lesson dicts, verdict fields included, so a project's own join reads that instead of the queue YAML. Pinned in `lessons_selftest.py` (verdicts arm).

## 1.17 (2026-09-22) — The maintainer's verdict comes back to the project: `reconcile` stamps status/imp_id/fixed_in from the plugin's shipped VERDICTS.yaml at every close, a recurrence of a resolved lesson reopens it, and `list` shows the verdict beside each id

- `lessons.py`: `LESSON_STATUSES` grows to open | collected | triaged | resolved | wontfix | dismissed; three optional fields `imp_id`, `fixed_in`, `verdict_at` (LESSONS.schema.yaml documents them; `validate` type-checks them and refuses an unknown status - fixtures `_smoke/07_verdicts.yaml` and `08_bad_verdict.yaml`). New `reconcile [--plugin-root]` reads `<plugin>/skills/lesson/VERDICTS.yaml`, matches rows on the marker's `project_uuid` / `project_id` or the legacy path hash, and stamps through one `apply_verdict` rule shared with the maintainer's `collect_lessons.py --mark`: idempotent, touches only named lessons, never re-closes a lesson that recurred after the very verdict it records (a newer fix does). `record-run` calls it at every close and prints one `Verdicts:` line when anything changed. `stamp_recurrence` sets a resolved / wontfix / dismissed lesson back to `open` and keeps the verdict fields - the reopen signal. `list` prints `[triaged IMP-041]`, `[resolved 0.9.8]`, `[resolved 0.9.17 - not installed]`, `[wontfix IMP-039]`, `[dismissed]`, `[open again after 0.9.8]` and takes `--status`. Pinned by `_smoke/lessons_selftest.py` (verdicts arm).
- Collector side (`lessons/collect_lessons.py`): `--mark` maps LEDGER.yaml onto every registered repo's queue (resolved -> resolved + imp_id + fixed_in; open/verify/planned -> triaged; wontfix; dismissed; a duplicate follows `duplicate_of`), `_fold_resent` carries the reopen fields to the inbox copy, and the digest lists REOPEN candidates. New `lessons/export_verdicts.py` writes the shipped manifest (no slug, no title; `on` kept while a verdict is unchanged; `--check` for CI). Pinned by `_smoke/collect_selftest.py` (three new arms) and the repo-level `verdict_close_selftest.py`.
- `references/lessons-capture.md`: "Delivery is not triage" now says how the verdict arrives and that a repeat after the fix is a reopen, never a hand-set verdict.
