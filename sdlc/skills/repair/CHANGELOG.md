...
## 1.25 (2026-09-26) — doctor's "should be reopened" hint needs a drifted upstream in the finding's own touched families: an empty `artifacts_touched` suppresses it (and is counted), and the filter runs before the first mismatch is picked

- `doctor.py` `resolved_stale_registry` (ledger IMP-195, aicf LSN-100 recurring on 0.9.16 via FND-087): a legacy resolved re-invoke finding with an empty `artifacts_touched` no longer draws the unfiltered reopen hint - `--provenance` prints one count line naming the skipped findings and `--json` carries them under `provenance.reopen_hints_skipped`, never inside `provenance.stale`. The family filter now runs BEFORE the first drifted upstream is picked, so an unrelated drift listed earlier no longer hides the finding's own. Upgrade effect: a failing check that was held under such a legacy finding now mints its own finding. Pinned by `_smoke/doctor_selftest.py` (empty-touched and ordering arms).
- Phase 5 clearance and the drain's pre-existing test compare `doctor.py --provenance --json` stale PAIRS against the Phase 2 snapshot pair by pair; `docs_index.py --stale` is only the human read-out and the source of the `re-stamp only` label (ledger IMP-222, aicf LSN-111: 21 pairs vs 6 rows read as a total clearance twice). Same rule in `references/orchestration.md` "Between waves" and `references/forward-propagation.md`. Pinned by `_smoke/orchestration_selftest.py`.
- The writer side of a measured handoff (`references/forward-propagation.md`, and SKILL.md's compact restatement): a measured note states only the window it measured and never claims the receiving file's whole delta; "expect re-stamp only" only when that window IS the stamp window (ledger IMP-226; pinned by `ux/_smoke/handoff_scope_selftest.py`).
- The Phase 5 re-slice verification line (and its `references/forward-propagation.md` copy) names `--container <cid|TASKS>`, so a finding on the system task file has its command (ledger IMP-224; pinned by `task/_smoke/authoring_prose_selftest.py`).
